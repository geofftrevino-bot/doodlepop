...
| Character | ElevenLabs voice | Voice ID | Direction | Pitch | Pace |
|---|---|---|---|---|---|
...
picks — they're unique to this channel and won't appear on anyone else's.

**These four are now canon.** Paste each voice ID into the table above — a
display name can be renamed or duplicated, but the ID is permanent, and for a
Voice Design creation it is the only way back to that exact voice. Re-running
the same design prompt produces a different voice. Never swap a voice between
episodes; a character who changes voice reads as a different character.

⚠️ **Confirm all four are saved to your Voice Library.** An unsaved Voice Design
generation cannot be recovered once the session ends.
...
## SIGN-OFF & CATCHPHRASE (supersedes "Doodle-doodle-pop!")

| Line | Job | When |
|---|---|---|
| **"We can fix anything!"** | Sign-off | Last line of every Short, spoken by whoever is on screen |
| **"Let's fix it!"** | Catchphrase | The moment a problem is spotted, mid-episode |

**Why two lines:** not every Short has something broken in it. "Let's fix it!"
after a colours or counting Short is a non-sequitur; "We can fix anything!" is
the show's promise and fits after any episode.

**Record each once per character, reuse forever:**
...
**Existing Shorts:** the sign-off sits at the end of the mix, so swapping it is
an audio-only remix — no video regeneration. "We can fix anything!" runs about
0.3–0.5s longer than the old tag, so a few tight-tailed Shorts (2, 11, 14) may
need their last line nudged earlier.

**Channel identity:** "DoodlePop" stays in the channel name, title card and
theme sting. It just isn't the spoken closing line any more.
